...
**交付**：`UserRepository` / `FriendRepository` / `FriendRequestRepository` / `ConversationRepository` / TabView（Chats / Contacts / Me） / 用户搜索（按用户名） / 发送好友申请 / 收到的好友申请列表（同意 / 拒绝） / 已发送与历史申请展示 / 好友列表 / 会话列表（头像、display_name、last_message 预览、unread 角标） / 下拉刷新 / 头像用 `LazyImage`。

**测试**：搜索用户 → 发送好友申请 → 对方同意后双方好友列表刷新；对方拒绝后不进入好友列表并出现在历史申请；会话列表能展示服务端已有会话的头像、昵称、最后消息、时间和未读数；下拉刷新可更新列表；头像正常加载。
