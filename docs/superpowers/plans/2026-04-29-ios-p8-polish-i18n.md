...
- [x] **Step 5: Commit Dark Mode 验收清单（即使无修复）**
...
**Task 5 实现记录（2026-04-29）**

- 已完成：完成 Dark Mode 代码审计清单，重点检查固定颜色、背景色、badge、错误态、头像 placeholder、气泡、Lightbox 与图片占位。
- 验证：使用 `rg` 扫描 `Color.black/white/red/green`、`foregroundStyle(.white/.red/.secondary)`、`Color(uiColor:)` 与 `.background(Color...)`；抽读命中视图确认均为语义色或设计上合理的固定色。Task 4 刚完成的 app build 已通过，本 Task 未改业务代码。
- 实现中问题：本轮未启动模拟器逐屏截图，Dark Mode 结论基于代码审计；截图 / 真机外观确认留到 Task 11 的实机自测脚本继续覆盖。未发现需要修复的 contrast 或硬编码颜色问题，因此 Step 4 无代码提交。
...
- [x] **Step 3: 场景 B — logout / login 切换（UserSession 释放）**
...
- [x] **Step 4: 场景 C — 图片上传重试（ImageSendStage / Data 持有）**
...
- [x] **Step 5: 把验收结果记进本计划末尾"内存泄漏验收"节**
...
**Task 10 实现记录（2026-04-29）**

- 已完成：确认本机 `xcrun xctrace list templates` 可用，且存在 `Allocations` template；计划末尾已补充 Task 10 内存验收状态。
- 验证：首次在沙箱内运行 `xcrun xctrace list templates` 因无法写入 `~/Library/Caches/com.apple.dt.InstrumentsCLI/path_manager` 失败；授权后命令通过并列出 `Allocations`。
- 实现中问题：未完成 Instruments 手工验收。Task 10 需要 Xcode Instruments generation 标记、20 次聊天进出、5 次 logout/login、5 次图片失败重试，并人工读取 Persistent Bytes / 类型残留；当前会话无法可靠自动化这些 generation 与手工读数。因此本轮只记录“未验收”，不声明无泄漏通过。
...
**Task 11 实现记录（2026-04-29）**

- 已完成：确认本计划无剩余未标记 checkbox；检查 `tasks.md` / `prd.md` 后未发现 iOS P8 / iOS 客户端阶段状态条目，因此无需修改。
- 验证：本阶段已跑过的关键验证包括 haptic 定向单测、ViewModel i18n 定向测试、`xcodebuild build`、`build-for-testing`、`GoldenPathSmokeTests` 定向 UITest（1 test, 0 failures）、catalog 状态检查（`total=90, en_translated=90, stale=0`）。
- 实现中问题：按用户最新指令“非必要情况下不要进行全量测试”，Task 11 未跑 `$TEST` 全量单测与 `$UITEST` 全量 UI 测试；实机自测脚本与 Instruments 内存泄漏验收也未完成手工实测，详见文档末尾“实机偏差”与“内存泄漏验收”。
...
| 屏幕 | Light | Dark | 备注 |
|---|---|---|---|
| LoginView | ✅ 代码审计 | ✅ 代码审计 | `Form` / system text + red error |
| RegisterView | ✅ 代码审计 | ✅ 代码审计 | `Form` / system text + red error |
| ConversationsListView (empty) | ✅ 代码审计 | ✅ 代码审计 | `ContentUnavailableView` |
| ConversationsListView (loaded) | ✅ 代码审计 | ✅ 代码审计 | unread badge 红底白字合理 |
| ConversationsListView (error) | ✅ 代码审计 | ✅ 代码审计 | `StateView.error` + semantic secondary |
| ChatView (no messages) | ✅ 代码审计 | ✅ 代码审计 | `systemBackground` / `secondarySystemBackground` |
| ChatView (with messages) | ✅ 代码审计 | ✅ 代码审计 | self bubble accent + white text；peer bubble semantic bg |
| ChatView (typing) | ✅ 代码审计 | ✅ 代码审计 | typing 文案 `.secondary` |
| ChatView (failed retry) | ✅ 代码审计 | ✅ 代码审计 | error 图标 / 文案 `.red` |
| ImageMessageBubble | ✅ 代码审计 | ✅ 代码审计 | pending overlay 黑色透明层 + white spinner |
| Lightbox | ✅ 代码审计 | ✅ 代码审计 | 故意全屏黑底白 icon / close |
| ContactsView (loaded) | ✅ 代码审计 | ✅ 代码审计 | request badge 红底白字合理 |
| FriendsListView (empty) | ✅ 代码审计 | ✅ 代码审计 | `ContentUnavailableView` |
| FriendRequestsSheetView | ✅ 代码审计 | ✅ 代码审计 | secondary 文案 + bordered buttons |
| UserSearchSheetView | ✅ 代码审计 | ✅ 代码审计 | inline secondary 提示 / system list bg |
| UserDetailView (online) | ✅ 代码审计 | ✅ 代码审计 | `PresenceDot` green 固定色合理 |
| UserDetailView (offline) | ✅ 代码审计 | ✅ 代码审计 | secondary 状态文案 |
| MeView | ✅ 代码审计 | ✅ 代码审计 | `Form` / destructive role |
| ProfileEditView (default) | ✅ 代码审计 | ✅ 代码审计 | avatar placeholder semantic bg |
| ProfileEditView (uploading) | ✅ 代码审计 | ✅ 代码审计 | `ProgressView` system style |
| ProfileEditView (upload failed) | ✅ 代码审计 | ✅ 代码审计 | upload error `.red` |
...
- 场景 A（聊天页反复进出 20 次）：未执行 Instruments generation B/C/D 读数；判定：未验收
- 场景 B（logout/login 5 次）：未执行 Instruments generation E/F 读数；判定：未验收
- 场景 C（图片重试 5 次）：未执行 Instruments generation G/H 读数；判定：未验收

结论：未完成手工内存泄漏验收。本轮仅确认 `xctrace` / `Allocations` template 可用；未观察或采集到足以声明“无泄漏”的 Instruments 证据。
...
- Task 5 Dark Mode：未做真实 light/dark 截图矩阵，只做代码审计；未观察到需改色的源码问题。
- Task 8 i18n：未跑英文 Locale 下的 LoginSmokeTests；已用 catalog 状态检查 + build 验证 `en.lproj/Localizable.strings` 生成。
- Task 9 Golden path：未跑 `$UITEST` 全量套件；新增 `GoldenPathSmokeTests` 已定向通过。
- Task 10 Instruments：未完成 Allocations 三场景手工验收；已确认 `xctrace` 与 `Allocations` template 可用，但未采集 generation / Persistent Bytes 证据。
- Task 11 收尾：未跑 `$TEST` / `$UITEST` 全量回归；原因是用户明确要求非必要情况下不要进行全量测试，本轮以构建、定向单测、定向 UITest 和静态检查替代。
